class Strips(object):
    ...
    def __init__(self, name, preconds, effects, cost=1):
        ...
        self.cost = cost
    """
    14 definestheSTRIPSrepresentationforanaction:
    15 *nameisthenameoftheaction
    16 *preconds,thepreconditions,isfeature:valuedictionarythat
    musthold
    17 fortheactionto becarriedout
    18 *effectsisafeature:valuemapthatthisactionmakes
    19 true.Theactionchangesthevalueofanyfeaturespecified
    20 here,andleavesotherfeaturesunchanged
    *costisthecostoftheaction
     22 """
...
class STRIPS_domain(object):
    ...
    """Problemdomain
     34 feature_domain_dictis afeature:domaindictionary,
     35 mappingeachfeaturetoitsdomain
     36 actions
     37 """
...
delivery_domain=STRIPS_domain(
    ...
    'MW':boolean, 'RHM':boolean}, #feature:valuesdictionary
...
###blocksworld
def move(x, y, z):
    """stringforthe 'move' action"""
    return 'move_' + x + '_from_' + y + '_to_' + z

def on(x):
    """stringforthe 'on'feature"""
    return x + '_is_on'


def clear(x):
    """stringforthe 'clear'feature"""
    return 'clear_' + x
